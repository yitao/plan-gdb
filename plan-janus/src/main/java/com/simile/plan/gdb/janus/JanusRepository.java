...
public class JanusRepository {
    ...
    private String indexEsName = "search";
    ...
    private JanusDataSourceConfig janusDataSourceConfig;

    private JanusGremlinBuilder gb;

    public JanusRepository(JanusDataSourceConfig janusDataSourceConfig) {
    ...
    public List<Result> submit(String gql) throws ExecutionException, InterruptedException {
    ...
    public List<String> databases() throws ExecutionException, InterruptedException {
        return result2String(
                submit(gb.clear().getGraphNames())
        );
    }

    public boolean existsDatabase(String database) throws ExecutionException, InterruptedException {
        return databases().stream()
                .anyMatch(item -> database.equals(item));
    }

    public List<String> vertexes(String database) throws ExecutionException, InterruptedException {
        return result2String(
                submit(gb.clear().openMgt(database).showVertexLabels())
        );
    }

    public List<String> edges(String database) throws ExecutionException, InterruptedException {
        return result2String(
                submit(gb.clear().openMgt(database).showEdgeLabels())
        );
    }
    ...
    public boolean createGraph(String graph) throws ExecutionException, InterruptedException {
        submit(gb.clear().startWithTemplateConfiguration()
    ...
    private String buildCreatePropertyAndIndexGql(GdbSuo suo, Set<String> existFields, List<JanusIndex> indexFields) {
    ...
    private String getClassName(FieldType fieldType) {
    ...
    private List<JanusIndex> getIndexFields(GdbSuo suo, JanusIndexVo janusIndexVo) {
    ...
    private boolean fixedNeedIndexField(SchemaType type, String field) {
        if (type.isVertex()) {
    ...
    private boolean needIndex(SchemaType schemaType, SchemaField schemaField, JanusIndexVo janusIndexVo, JanusIndexType indexType) {
